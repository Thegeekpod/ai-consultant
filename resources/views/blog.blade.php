...
@section('title', 'Blog - AI Strategy & Insights')
            ...
            <div class="tp-blog-standard-list">
              @forelse($blogs as $blog)
                <div class="tp-blog-item">
                  ...
                  <p>{{ Str::limit(strip_tags($blog->description), 180) }}</p>
                </div>
              @empty
                <div class="tp-blog-item">
                  ...
                  @foreach($latestPosts as $lpost)
                    <div class="lpost-item">
                      <div class="categories">
                        <a href="#">{{ $lpost->category->name ?? 'Uncategorized' }}</a>
                      </div>
                      <h3 class="title"><a href="{{ route("blog-details", $lpost->slug) }}">{{ $lpost->title }}</a></h3>
                      <p class="date">{{ $lpost->created_at->format('d M, Y') }}</p>
                    </div>
                  @endforeach
                </div>
              </div>
