...
(function ($) {
  ...
  gsap.set(".tp--marquee-text-inner", { xPercent: 0 });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const box = { x: w * 0.7, y: h * 0.2, width: w * 0.2, height: h * 0.3 };
    ...
    section.addEventListener('mouseleave', () => { defaultAlpha = 0; });
  ...
  (function () {
    const journeys = [
      { left: "topPathL", right: "topPathR", dot: "dotTop", duration: 6000 },
      { left: "midPathL", right: "midPathR", dot: "dotMid", duration: 6000 },
      { left: "bottomPathL", right: "bottomPathR", dot: "dotBottom", duration: 6000 }
    ];
  ...
  linesAnimation.forEach(splitLines => {
    ...
    const textSplitLines = new SplitText(splitLines, { type: "lines" });
    gsap.set(splitLines, { perspective: 400 });
    textSplitLines.split({ type: "lines" })
  ...
  if (document.querySelector("#dotTop") && document.querySelector("#dotBottom")) {
    ...
    const moveTL = gsap.timeline({ repeat: -1 });
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (isScrollingDown && scroll > 100) {
          gsap.to(header_sticky, { y: -headerHeight, duration: 0.4, ease: "power2.out" });
        }

        if (isScrollingUp) {
          gsap.to(header_sticky, { y: 0, duration: 0.4, ease: "power2.out" });
  ...
  if (ticketArea && ticketBgImg && screen.width > 767) {
    ...
    ticketTL.fromTo(".tp-aai-conference-ticket-text",
      { opacity: 0 },
      {
        ...
        ease: "power2.out",
        onStart: () => {
          gsap.set(".tp-aai-conference-ticket-text", { display: "block" });
        }
      },
      "+=0.2"
    );
    ...
    ticketTL.fromTo(".tp-aai-conference-ticket-box",
      { opacity: 0 },
      {
        ...
        ease: "power2.out",
        onStart: () => {
          gsap.set(".tp-aai-conference-ticket-box", { display: "block" });
        }
      },
      "+=0.3"
    );
  ...
  function tpTextInvertAnimation() {
    ...
    const invertSplit = new SplitText(".tp-text-invert", { type: "lines" });
